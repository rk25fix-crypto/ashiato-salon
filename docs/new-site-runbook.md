...
- Instagram などの SNS の URL(**その場で実際に開いて、正しいアカウントか確かめる**。1号案件では綴りの誤りで存在しないアカウントに飛んでいた)
- 業種ごとの**法定表示**(例: ペット関係なら第一種動物取扱業の登録番号。1号案件では後から追加した)
- 写真(外観・店内・作品やサービスの例)。スマホ写真でよい
- **変更をオーナーが自分で直したいか、こちらに頼みたいか**。答えを記録しておく(1号案件のオーナーは「頼めばいい」派だった)
- よく変わる所(料金・お知らせ・写真の入れ替えなど)。自分で直す派なら、ここが編集できる項目になる
- 事例として紹介してよいか、紹介をお願いしてよいか

聞き方の注意: 「どうしますか？」と聞くと、相手はゴールが決まっていないまま「はい」と答えがち。**「A と B ならどちらが近いですか？」と選んでもらう**。「はい」だけで合意したことにしない。

## 1.5 デザインのモックを作って見てもらう

相手はたたき台を見て初めて考えられるので、修正の往復は避けられない。往復を**この段階に集めて**、実装後の「80%からの終わらない調整」を減らす。

1. ヒアリング内容で、見た目だけのモック(1枚の HTML、または画像)を作る。配色や雰囲気は2案ほど出して選んでもらう
2. スマホで見せて、反応を見る(言葉の「はい」より表情や質問を重視する)
3. 構成・配色・載せる内容がここで決まったら、実装に入る。**実装後の修正は回数と範囲をあらかじめ伝えておく**(例: 文言・写真の差し替えは何度でも、構成の変更は1回まで)
...
- [ ] SNS・LINE・地図のリンクとQRコードが、すべて正しい相手に飛ぶ
...
1. 書き換える所
   ...
   - 連絡先(`<p class="name">` の1行)。お客さまが来た経路で変える
     - ココナラ経由: `ココナラのメッセージ(トークルーム)` とする。ココナラの規約で外部の連絡先(LINE・メール・電話)を渡せないため、**LINE などは書かない**
     - 紹介・直接提案: LINE公式アカウント(知人は個人の LINE でもよい)
...
- 使い方説明(PDF か印刷)と合言葉を渡す。**合言葉は紙か対面で。LINE などに残さない**
  - ココナラ経由で対面できない場合は、ココナラのメッセージで送るしかない(外部の連絡手段は使えない)。使い方説明とは別のメッセージにする
